...
test('missing production notification context is explicit, not a fabricated route', async () => {
  await expect(new PendingReminderContextService().get()).rejects.toMatchObject(
    { feature: 'Production reminder delivery context' },
  );
});
